                                ...
                                if (!empty($this->Items)) {
                                    ...
                                    foreach ($this->Items as $key => $value) {
                                        ...
                                        if(file_exists($picturePath) == true){
                                            $picture    = '<img style="width:90px;height:120px;" src="'.UPLOAD_URL . 'blog' . DS . '448x527-' . $value['picture'].'">';                                    
                                        }else{
                                            $picture    = '<img style="width:90px;height:120px;" src="'.UPLOAD_URL . 'blog' . DS . '60x90-default.jpg' .'">';        
                                        }
                                                       
                                        $row            = ($i % 2 == 0) ? 'even' : 'odd';
                                        $status         = Helper::cmsStatus($value['status'], URL::createLink('admin', 'blog', 'ajaxStatus', array('id' => $id, 'status' => $value['status'])), $id);                                
                                        $created        = Helper::formatDate('d-m-Y', $value['created']);
                                        $modified       = Helper::formatDate('d-m-Y', $value['modified']);
                                        $created_by     = $value['created_by'];
                                        $modified_by    = $value['modified_by'];
                                        $linkEdit       = URL::createLink('admin', 'blog', 'form', array('id' => $id));
